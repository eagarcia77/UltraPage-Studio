...
export default function Home() {
  ...
  const handleRibbonKeyDown = (event: React.KeyboardEvent<HTMLButtonElement>) => {
    const tabs = ["home", "insert", "layout", "review"] as const;
          ...
          <div className={`ribbon-shell ${mode === "visual" ? "" : "ribbon-compact"}`}>
            <div className="ribbon-nav">
              ...
              {mode === "visual" && <div className="ribbon-tabs" role="tablist" aria-label="Editor ribbon">
                {(["home", "insert", "layout", "review"] as const).map((tab) => <button key={tab} type="button" role="tab" data-ribbon-tab={tab} aria-selected={ribbonTab === tab} tabIndex={ribbonTab === tab ? 0 : -1} className={ribbonTab === tab ? "active" : ""} onKeyDown={handleRibbonKeyDown} onClick={() => setRibbonTab(tab)}>{tab[0].toUpperCase() + tab.slice(1)}</button>)}
              </div>}
            ...
            {mode === "visual" && <div className="ribbon-panel" role="tabpanel" aria-label={`${ribbonTab} ribbon`}>
              {ribbonTab === "home" && <>
              ...
              {ribbonTab === "layout" && <>
                ...
                <div className="ribbon-group"><div className="ribbon-group-body"><div className="ribbon-select-row"><label className="toolbar-select-label line-spacing-select"><span className="sr-only">Line spacing</span><select defaultValue="" onChange={(event) => applyLineSpacing(event.target.value)} aria-label="Line spacing"><option value="" disabled>Line spacing</option><option value="1">1.0</option><option value="1.15">1.15</option><option value="1.5">1.5</option><option value="2">2.0 double</option><option value="2.5">2.5</option></select></label><label className="toolbar-select-label indentation-select"><span className="sr-only">Paragraph indentation</span><select defaultValue="" onChange={(event) => applyIndentation(event.target.value)} aria-label="Paragraph indentation"><option value="" disabled>Indentation</option><option value="first-line">First line (0.5″)</option><option value="left">Entire paragraph (0.5″)</option><option value="hanging">Hanging indent (0.5″)</option><option value="none">Remove indentation</option></select></label></div><p className="ribbon-hint">Paragraphs with 4+ sentences receive an automatic 0.5″ first-line indent.</p></div><span className="ribbon-group-label">Paragraph Layout</span></div>
