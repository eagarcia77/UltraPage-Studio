...
export default function Home() {
  ...
  const downloadDocument = () => {
    ...
    toast.success("File downloaded en la computadora");
  };
  useEffect(() => {
...
function RubricDialog({ insertMarkup }: { insertMarkup: (markup: string) => void }) {
  ...
  const create = () => {
    ...
    const safeTitle = escapeHtml(title.trim() || "Assessment Rubric");
...
function ContentDialog({ trigger, search, setSearch, files, insertFile, documentHtml, documentFileName, openDocument, newDocument, documentLanguage, documentAuthor, documentDescription }: ContentDialogProps) {
  ...
  const uploadEditedFile = async (file?: File) => {
    ...
    try {
      ...
      toast.success(exists ? "Archivo reemplazado en WebDAV" : "File uploaded a WebDAV", { description: file.name });
      await connect(url);
