...
function ModuleTemplateDialog({ insertMarkup, hasH1 }: { insertMarkup: (markup: string) => void; hasH1: boolean }) {
  ...
  const insert = () => {
    ...
    const label = moduleNumber.trim() ? `MODULE ${escapeHtml(moduleNumber.trim())}` : "MODULE";
...
function RubricDialog({ insertMarkup }: { insertMarkup: (markup: string) => void }) {
  ...
  const create = () => {
    ...
    const rows = Array.from({ length: safeCriteria }, (_, criterionIndex) => {
      const cells = names.map((_, levelIndex) => `<td><strong>${safeLevels - levelIndex} pts</strong><br>Describe the expected performance.</td>`).join("");
      return `<tr><th scope="row">Criterion ${criterionIndex + 1}</th>${cells}</tr>`;
    }).join("");
    insertMarkup(`<table class="rubric-table" data-table-style="grid" aria-label="${safeTitle}"><caption>${safeTitle}</caption><thead><tr><th scope="col">Criterion</th>${headings}</tr></thead><tbody>${rows}</tbody></table><p class="rubric-total"><strong>Maximum score:</strong> ${safeCriteria * safeLevels} points</p>`);
    setOpen(false);
  };
...
function TableDialog({ insertMarkup, block = false }: { insertMarkup: (markup: string) => void; block?: boolean }) {
  ...
  const createTable = () => {
    ...
    const markup = tableStyle === "apa7"
      ? `<p class="apa-table-heading"><strong>Table 1</strong><br><em>${safeTitle}</em></p>${table}<p class="apa-table-note"><em>Note.</em> Add a table note if needed.</p>`
      : table;
    insertMarkup(markup);
    setOpen(false);
