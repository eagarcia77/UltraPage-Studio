...
export default function Home() {
  ...
  const [ribbonTab, setRibbonTab] = useState<"home" | "insert" | "layout" | "references" | "review" | "view">("home");
    ...
    <div className="workspace">
      ...
      <section className="editor-shell">
        ...
        <Tabs value={mode} onValueChange={changeMode} className="editor-tabs">
          <div className={`ribbon-shell ${mode === "visual" ? "" : "ribbon-compact"}`}>
            <div className="ribbon-nav">
              ...
              {mode === "visual" && <div className="ribbon-tabs" role="tablist" aria-label="Editor ribbon">
                {(["home", "insert", "layout", "references", "review", "view"] as const).map((tab) => <button key={tab} id={`ribbon-tab-${tab}`} type="button" role="tab" data-ribbon-tab={tab} aria-controls="ribbon-panel" aria-selected={ribbonTab === tab} tabIndex={ribbonTab === tab ? 0 : -1} className={ribbonTab === tab ? "active" : ""} onKeyDown={handleRibbonKeyDown} onClick={() => setRibbonTab(tab)}>{tab[0].toUpperCase() + tab.slice(1)}</button>)}
              </div>}
              {mode === "visual" && <div className="ribbon-quick" role="group" aria-label="Quick access"><button type="button" onClick={() => command("undo")} aria-label="Undo" title="Undo"><Undo2 /></button><button type="button" onClick={() => command("redo")} aria-label="Redo" title="Redo"><Redo2 /></button></div>}
            </div>
            {mode === "visual" && <div id="ribbon-panel" className="ribbon-panel" role="tabpanel" aria-labelledby={`ribbon-tab-${ribbonTab}`}>
              {ribbonTab === "home" && <>
                <div className="ribbon-group ribbon-clear-group"><div className="ribbon-group-body"><button type="button" className="ribbon-command ribbon-large" onClick={clearFormatting} aria-label="Clear formatting" title="Clear formatting from the selected text"><Eraser/><span>Clear</span></button></div><span className="ribbon-group-label">Formatting</span></div>
                <div className="ribbon-group ribbon-font-group"><div className="ribbon-group-body"><div className="ribbon-select-row"><label className="toolbar-select-label font-family-select"><span className="sr-only">Font family</span><select defaultValue="" onChange={(event) => applyFont(event.target.value)} aria-label="Font family"><option value="" disabled>Font family</option><option value="Arial">Arial</option><option value="Calibri">Calibri</option><option value="Georgia">Georgia</option><option value="Tahoma">Tahoma</option><option value="Times New Roman">Times New Roman</option><option value="Verdana">Verdana</option></select></label><label className="toolbar-select-label font-size-select"><span className="sr-only">Font size</span><select defaultValue="" onChange={(event) => applyFontSize(event.target.value)} aria-label="Font size"><option value="" disabled>Size</option><option value="10">10 px</option><option value="12">12 px</option><option value="14">14 px</option><option value="16">16 px</option><option value="18">18 px</option><option value="24">24 px</option><option value="32">32 px</option><option value="40">40 px</option></select></label></div><div className="ribbon-icon-row"><button type="button" onClick={() => command("bold")} aria-label="Bold" title="Bold"><Bold/></button><button type="button" onClick={() => command("italic")} aria-label="Italic" title="Italic"><Italic/></button><button type="button" onClick={() => command("underline")} aria-label="Underline" title="Underline"><Underline/></button><button type="button" onClick={() => command("strikeThrough")} aria-label="Strikethrough" title="Strikethrough"><Strikethrough/></button><button type="button" onClick={() => command("subscript")} aria-label="Subscript" title="Subscript"><Subscript/></button><button type="button" onClick={() => command("superscript")} aria-label="Superscript" title="Superscript"><Superscript/></button></div></div><span className="ribbon-group-label">Font</span></div>
                <div className="ribbon-group ribbon-paragraph-group"><div className="ribbon-group-body"><div className="ribbon-icon-row"><button type="button" onClick={() => command("insertUnorderedList")} aria-label="Bulleted list" title="Bulleted list"><List/></button><button type="button" onClick={() => command("insertOrderedList")} aria-label="Numbered list" title="Numbered list"><ListOrdered/></button><button type="button" onClick={() => command("outdent")} aria-label="Decrease indent" title="Decrease indent"><Rows3 className="indent-decrease"/></button><button type="button" onClick={() => command("indent")} aria-label="Increase indent" title="Increase indent"><Rows3/></button><button type="button" onClick={() => command("justifyLeft")} aria-label="Align left" title="Align left"><AlignLeft/></button><button type="button" onClick={() => command("justifyCenter")} aria-label="Center" title="Center"><AlignCenter/></button><button type="button" onClick={() => command("justifyRight")} aria-label="Align right" title="Align right"><AlignRight/></button><button type="button" onClick={() => command("justifyFull")} aria-label="Justify" title="Justify"><AlignJustify/></button></div><div className="ribbon-select-row"><label className="toolbar-select-label line-spacing-select"><span className="sr-only">Line spacing</span><select defaultValue="" onChange={(event) => applyLineSpacing(event.target.value)} aria-label="Line spacing"><option value="" disabled>Line spacing</option><option value="1">1.0</option><option value="1.15">1.15</option><option value="1.5">1.5</option><option value="2">2.0 double</option><option value="2.5">2.5</option></select></label><label className="toolbar-select-label indentation-select"><span className="sr-only">Paragraph indentation</span><select defaultValue="" onChange={(event) => applyIndentation(event.target.value)} aria-label="Paragraph indentation"><option value="" disabled>Indentation</option><option value="first-line">First line (0.5″)</option><option value="left">Entire paragraph (0.5″)</option><option value="hanging">Hanging indent (0.5″)</option><option value="none">Remove indentation</option></select></label></div></div><span className="ribbon-group-label">Paragraph</span></div>
                <div className="ribbon-group ribbon-styles-group"><div className="ribbon-group-body ribbon-style-gallery">{[["p","Paragraph"],["h1","H1"],["h2","H2"],["h3","H3"],["h4","H4"]].map(([tag,label]) => <button type="button" key={tag} onClick={() => command("formatBlock", tag)}><span className={`style-preview style-${tag}`}>{label}</span></button>)}</div><span className="ribbon-group-label">Styles</span></div>
              ...
              {ribbonTab === "layout" && <>
                ...
                <div className="ribbon-group"><div className="ribbon-group-body ribbon-command-row"><TableEditDialog editTable={editSelectedTable}/><button type="button" className="ribbon-command" onClick={() => command("insertHorizontalRule")}><Minus/><span>Divider</span></button></div><span className="ribbon-group-label">Arrange</span></div>
              </>}
              ...
              {ribbonTab === "review" && <>
                ...
                <div className="ribbon-group"><div className="ribbon-group-body ribbon-command-row"><ApaDialog insertMarkup={insertMarkup} language={documentLanguage}/><button type="button" className="ribbon-command" onClick={() => { setSidePanelTab("review"); setRightPanel(true); }}><Check/><span>Checks</span></button></div><span className="ribbon-group-label">Academic Review</span></div>
                <div className="ribbon-group"><div className="ribbon-group-body ribbon-document-stats"><strong>{wordCount}</strong><span>words</span><strong>{Math.max(1, Math.ceil(wordCount / 200))} min</strong><span>reading time</span></div><span className="ribbon-group-label">Document Statistics</span></div>
              </>}
              {ribbonTab === "view" && <>
                ...
                <div className="ribbon-group"><div className="ribbon-group-body ribbon-command-row"><button type="button" className="ribbon-command" onClick={() => setRulerUnit((current) => current === "in" ? "cm" : "in")}><Columns3/><span>{rulerUnit === "in" ? "Inches" : "Centimeters"}</span></button><button type="button" className={`ribbon-command ${rightPanel ? "is-active" : ""}`} aria-pressed={rightPanel} onClick={() => setRightPanel((visible) => !visible)}><PanelRight/><span>Tools Panel</span></button><button type="button" className="ribbon-command" onClick={() => { setSidePanelTab("outline"); setRightPanel(true); }}><Heading2/><span>Outline</span></button></div><span className="ribbon-group-label">Workspace</span></div>
                <div className="ribbon-group"><div className="ribbon-group-body ribbon-command-row"><button type="button" className="ribbon-command" onClick={() => changeMode("ultra")}><Monitor/><span>Final Preview</span></button><button type="button" className="ribbon-command" onClick={() => changeMode("html")}><Code2/><span>HTML</span></button></div><span className="ribbon-group-label">Document Views</span></div>
              </>}
            </div>}
          </div>
          <TabsContent value="visual" className="canvas-wrap"><div className={`device-frame ${device}`}><div className="ultra-label"><span className="mini-logo" aria-hidden="true"><img src="/brand/ultrapage-mark.svg" alt="" /></span><span>Design Preview</span><span className="ruler-status">Rulers: {rulerUnit === "in" ? "inches" : "centimeters"}</span></div><EditorRulers unit={rulerUnit} device={device} onToggle={() => setRulerUnit((current) => current === "in" ? "cm" : "in")}><div ref={attachEditor} className={`page-canvas ${imageDragActive ? "image-drag-active" : ""}`} contentEditable spellCheck={spellCheckEnabled} suppressContentEditableWarning onPaste={handlePaste} onDragEnter={handleImageDragOver} onDragOver={handleImageDragOver} onDragLeave={() => setImageDragActive(false)} onDrop={handleImageDrop} onInput={handleEditorInput} aria-label="Editable page content" /></EditorRulers></div></TabsContent>
