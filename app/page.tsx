          ...
          <div className={`ribbon-shell ${mode === "visual" ? "" : "ribbon-compact"} ${ribbonCollapsed ? "ribbon-collapsed" : ""}`}>
            ...
            {mode === "visual" && !ribbonCollapsed && <div id="ribbon-panel" className="ribbon-panel" role="tabpanel" aria-labelledby={`ribbon-tab-${ribbonTab}`}>
              {ribbonTab === "file" && <>
                ...
                <div className="ribbon-group"><div className="ribbon-group-body ribbon-command-row"><button type="button" className="ribbon-command" onClick={copyHtml}><Copy/><span>Copy for {activeLms.shortLabel}</span></button><button type="button" className="ribbon-command" onClick={() => changeMode("ultra")}><Monitor/><span>Final Preview</span></button><button type="button" className="ribbon-command" onClick={() => changeMode("html")}><Code2/><span>HTML</span></button></div><span className="ribbon-group-label">Publish</span></div>
                <div className="ribbon-group"><div className="ribbon-group-body ribbon-file-profile"><label><span>Target LMS</span><select value={lmsProfile} onChange={(event) => { setLmsProfile(event.target.value as LmsProfile); setSaved(false); }}>{Object.entries(lmsProfiles).map(([value, profile]) => <option key={value} value={value}>{profile.label}</option>)}</select></label><small>{activeLms.guidance}</small></div><span className="ribbon-group-label">Compatibility Profile</span></div>
              </>}
              {ribbonTab === "home" && <>
