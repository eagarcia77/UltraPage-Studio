...
function ModuleTemplateDialog({ insertMarkup, hasH1 }: { insertMarkup: (markup: string) => void; hasH1: boolean }) {
  ...
  return <Dialog open={open} onOpenChange={setOpen}><DialogTrigger asChild><button className="template-card"><span className="template-icon blue"><BookOpen/></span><span><strong>Módulo completo</strong><small>Estructura accesible para Ultra</small></span><Plus size={16}/></button></DialogTrigger><DialogContent className="module-template-dialog"><DialogHeader><DialogTitle>Crear estructura de módulo</DialogTitle><DialogDescription>Genera una plantilla organizada y ajusta automáticamente el nivel de los encabezados.</DialogDescription></DialogHeader><div className="module-template-fields"><label>Número o identificación<Input value={moduleNumber} onChange={(event) => setModuleNumber(event.target.value)} placeholder="4"/></label><label>Título del módulo<Input value={moduleTitle} onChange={(event) => setModuleTitle(event.target.value)} placeholder="Tecnología, personas y procesos"/></label></div>{hasH1 && <p className="field-help">El documento ya tiene un H1; el módulo comenzará con H2 para conservar la jerarquía.</p>}<div className="apa-actions"><Button variant="outline" onClick={() => setOpen(false)}>Cancelar</Button><Button onClick={insert}><BookOpen size={16}/> Crear módulo</Button></div></DialogContent></Dialog>;
}
...
function accessibilityReport(html: string, title: string, language: DocumentLanguage = "es-PR"): AccessibilityCheck[] {
  ...
  return [
    ...
    { ok: imagesAccessible, text: images.length ? "Las imágenes informativas tienen texto alternativo y las decorativas están identificadas correctamente" : "No hay imágenes que requieran texto alternativo" },
...
function RubricDialog({ insertMarkup }: { insertMarkup: (markup: string) => void }) {
  ...
  return <Dialog open={open} onOpenChange={setOpen}><DialogTrigger asChild><Button variant="outline" className="rubric-trigger"><Table2 size={16}/> Crear rúbrica</Button></DialogTrigger><DialogContent className="rubric-dialog"><DialogHeader><DialogTitle>Generador de rúbrica accesible</DialogTitle><DialogDescription>Crea una tabla con encabezados de fila y columna compatible con Blackboard, HTML, Word y PDF.</DialogDescription></DialogHeader><div className="rubric-fields"><label>Título<Input value={title} onChange={(event) => setTitle(event.target.value)}/></label><label>Criterios<Input type="number" min="1" max="12" value={criteria} onChange={(event) => setCriteria(Number(event.target.value))}/></label><label>Niveles<Input type="number" min="2" max="6" value={levels} onChange={(event) => setLevels(Number(event.target.value))}/></label><label className="rubric-levels">Nombres de los niveles<Input value={levelNames} onChange={(event) => setLevelNames(event.target.value)} placeholder="Excelente, Competente, En desarrollo, Inicial"/><span>Separe los niveles con comas.</span></label></div><div className="apa-actions"><Button variant="outline" onClick={() => setOpen(false)}>Cancelar</Button><Button onClick={create}><Table2 size={16}/> Insertar rúbrica</Button></div></DialogContent></Dialog>;
}
