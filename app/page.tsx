...
export default function Home() {
  ...
  const copyHtml = async () => {
    ...
    try {
      ...
      try {
        await navigator.clipboard.writeText(blackboardMarkup);
        toast.success("Compatible HTML code copied", { description: "El navegador utilizó el modo compatible. Paste it into Blackboard Ultra’s HTML <> editor." });
      } catch {
  ...
  const insertFile = (name: string, type: string, href?: string) => {
    ...
    command("insertHTML", markup); toast.success("Resource inserted", { description: `${name} se añadió a la página.` });
  };
  ...
  return <main className="min-h-screen bg-[#f4f6f9] text-[#172033]">
    ...
    <div className="workspace">
      ...
      <section className="editor-shell">
        <div className="document-head"><div><div className="breadcrumbs"><span>Standalone editor</span><span>/</span><span>Document</span></div><input className="title-input" value={title} onChange={(e) => { setTitle(e.target.value); setSaved(false); }} aria-label="Título de la página" /><div className="document-metrics" aria-live="polite"><span>{wordCount} words</span><span>{characterCount} characters</span><span>Autosave active</span></div><label className="document-language"><span>Document language</span><select value={documentLanguage} onChange={(event) => { setDocumentLanguage(event.target.value as DocumentLanguage); setSaved(false); }} aria-label="Idioma principal del documento"><option value="es-PR">Español (Puerto Rico)</option><option value="en-US">English (United States)</option></select></label></div><div className="view-controls" aria-label="Device preview"><button onClick={() => setDevice("desktop")} className={device === "desktop" ? "active" : ""} aria-label="Desktop"><Monitor size={17}/></button><button onClick={() => setDevice("tablet")} className={device === "tablet" ? "active" : ""} aria-label="Tablet"><Tablet size={17}/></button><button onClick={() => setDevice("mobile")} className={device === "mobile" ? "active" : ""} aria-label="Mobile"><Smartphone size={17}/></button><button onClick={() => setRightPanel(!rightPanel)} className={rightPanel ? "active panel-toggle" : "panel-toggle"} aria-label="Show or hide panel" aria-expanded={rightPanel} aria-controls="editor-side-panel"><PanelRight size={17}/></button></div><button type="button" className="mobile-panel-toggle" onClick={() => setRightPanel(true)} aria-expanded={rightPanel} aria-controls="editor-side-panel"><PanelRight size={16}/> Tools</button></div>
        <Tabs value={mode} onValueChange={changeMode} className="editor-tabs">
          <div className="toolbar-row">
            ...
            {mode === "visual" && <div className="toolbar" role="toolbar" aria-label="Formato de texto">
              ...
              <label className="toolbar-select-label line-spacing-select"><span className="sr-only">Line spacing</span><select defaultValue="" onChange={(event) => applyLineSpacing(event.target.value)} aria-label="Line spacing"><option value="" disabled>Line spacing</option><option value="1">1.0</option><option value="1.15">1.15</option><option value="1.5">1.5</option><option value="2">2.0 doble</option><option value="2.5">2.5</option></select></label>
          ...
          {mode === "visual" && <div className="secondary-toolbar" role="toolbar" aria-label="Alignment, indentation, tables, and watermark"><button className="clear-format-button" onClick={clearFormatting} aria-label="Clear formatting" title="Clear all formatting from selected text"><Eraser /><span>Clear formatting</span></button><span className="secondary-divider"/><TableDialog insertMarkup={insertMarkup}/><TableEditDialog editTable={editSelectedTable}/><WatermarkDialog applyWatermark={applyWatermark} removeWatermark={removeWatermark}/><span className="secondary-divider"/><label className="toolbar-select-label indentation-select"><span className="sr-only">Paragraph indentation</span><select defaultValue="" onChange={(event) => applyIndentation(event.target.value)} aria-label="Paragraph indentation"><option value="" disabled>Paragraph indentation</option><option value="first-line">First line (0.5″)</option><option value="left">Entire paragraph (0.5″)</option><option value="hanging">Hanging indent (0.5″)</option><option value="none">Remove indentation</option></select></label><span className="secondary-divider"/><button onClick={() => command("justifyLeft")} aria-label="Align left" title="Align left"><AlignLeft /></button><button onClick={() => command("justifyCenter")} aria-label="Center" title="Center"><AlignCenter /></button><button onClick={() => command("justifyRight")} aria-label="Align right" title="Align right"><AlignRight /></button><button onClick={() => command("justifyFull")} aria-label="Justify" title="Justify"><AlignJustify /></button></div>}
...
function ExportDialog({ html, title, language, author, description, downloadHtml }: { html: string; title: string; language: DocumentLanguage; author: string; description: string; downloadHtml: () => void }) {
  ...
  const exportDocument = async (format: "docx" | "pdf") => {
    ...
    try {
      ...
      toast.error("Export failed", { description: problem instanceof Error ? problem.message : "Intente nuevamente." });
...
function MediaDialog({ insertMedia }: { insertMedia: (options: { url: string; title: string; transcript: string; captions: boolean }) => boolean }) {
  ...
  return <Dialog open={open} onOpenChange={setOpen}><DialogTrigger asChild><button aria-label="Insert Accessible Video" title="Insert Accessible Video"><Video/></button></DialogTrigger><DialogContent className="media-dialog"><DialogHeader><DialogTitle>Insert Accessible Video</DialogTitle><DialogDescription>Supports HTTPS links from YouTube, Vimeo, and Kaltura. The video must include captions or a transcript.</DialogDescription></DialogHeader><div className="media-dialog-grid"><label>Video URL<Input value={url} onChange={(event) => setUrl(event.target.value)} placeholder="https://youtu.be/…"/></label><label>Descriptive title<Input value={title} onChange={(event) => setTitle(event.target.value)} placeholder="Introducción al módulo 4"/></label><label>Optional transcript URL<Input value={transcript} onChange={(event) => setTranscript(event.target.value)} placeholder="https://…/transcripcion.pdf"/></label><label className="checkbox-label"><input type="checkbox" checked={captions} onChange={(event) => setCaptions(event.target.checked)}/> The video includes synchronized captions</label></div><div className="apa-actions"><Button variant="outline" onClick={() => setOpen(false)}>Cancel</Button><Button onClick={insert}><Video size={16}/> Insert Video</Button></div></DialogContent></Dialog>;
}
...
function RubricDialog({ insertMarkup }: { insertMarkup: (markup: string) => void }) {
  ...
  const create = () => {
    ...
    const defaultNames = ["Excelente", "Competente", "En desarrollo", "Inicial", "Insuficiente", "No evidenciado"];
    const names = Array.from({ length: safeLevels }, (_, index) => provided[index] || defaultNames[index] || `Nivel ${index + 1}`);
    const safeTitle = escapeHtml(title.trim() || "Rúbrica de evaluación");
...
function TableDialog({ insertMarkup, block = false }: { insertMarkup: (markup: string) => void; block?: boolean }) {
  ...
  const createTable = () => {
    ...
    const headingCells = Array.from({ length: safeColumns }, (_, index) => `<th scope="col">Header ${index + 1}</th>`).join("");
    const bodyRows = Array.from({ length: safeRows }, () => `<tr>${Array.from({ length: safeColumns }, () => "<td>Data</td>").join("")}</tr>`).join("");
    const safeTitle = clean(title.trim() || "Table title");
...
function ApaDialog({ insertMarkup, fullWidth = false }: { insertMarkup: (markup: string) => void; fullWidth?: boolean }) {
  ...
  return <Dialog><DialogTrigger asChild><Button variant="outline" className={fullWidth ? "apa-trigger full" : "apa-trigger"}><BookOpen size={16}/> APA 7</Button></DialogTrigger><DialogContent className="apa-dialog"><DialogHeader><DialogTitle>APA 7 Tools</DialogTitle><DialogDescription>Inserts academic elements when the course page requires them.</DialogDescription></DialogHeader><Tabs defaultValue="citation"><TabsList className="apa-tabs"><TabsTrigger value="citation"><Quote size={14}/> Cita</TabsTrigger><TabsTrigger value="reference"><BookOpen size={14}/> Referencia</TabsTrigger><TabsTrigger value="table"><Table2 size={14}/> Tabla</TabsTrigger><TabsTrigger value="figure"><ImagePlus size={14}/> Figura</TabsTrigger></TabsList><TabsContent value="citation" className="apa-pane"><div className="form-grid"><label>Author or authors<Input value={author} onChange={(e)=>setAuthor(e.target.value)}/></label><label>Year<Input value={year} onChange={(e)=>setYear(e.target.value)}/></label><label>Optional page<Input value={page} onChange={(e)=>setPage(e.target.value)} placeholder="45"/></label></div><div className="apa-preview"><small>Preview</small><p>{citation}</p></div><div className="apa-actions"><Button variant="outline" onClick={()=>insertMarkup(`<p>${author} (${year}) sostiene que [escriba aquí la idea]${page ? ` (p. ${page})` : ""}.</p>`)}>Narrative citation</Button><Button onClick={()=>insertMarkup(`<span>${citation}</span>`)}>Parenthetical citation</Button></div></TabsContent><TabsContent value="reference" className="apa-pane"><label>Complete reference<Textarea value={reference} onChange={(e)=>setReference(e.target.value)} rows={5}/></label><p className="field-help">Review italics, capitalization, DOI, or URL according to the source type.</p><Button onClick={()=>insertMarkup(`<h2>Referencias</h2><p class="apa-reference">${reference}</p>`)}>Insert with hanging indent</Button></TabsContent><TabsContent value="table" className="apa-pane"><label>Table title<Input value={label} onChange={(e)=>setLabel(e.target.value)}/></label><div className="apa-preview table-preview"><strong>Tabla 1</strong><em>{label}</em><div>Header 1　 Header 2</div></div><Button onClick={()=>insertMarkup(`<figure class="apa-table"><p><strong>Tabla 1</strong><br><em>${label}</em></p><table data-table-style="apa7"><thead><tr><th scope="col">Header 1</th><th scope="col">Header 2</th></tr></thead><tbody><tr><td>Data</td><td>Data</td></tr></tbody></table><figcaption><em>Nota.</em> Describa aquí la información necesaria para interpretar la tabla.</figcaption></figure>`)}>Insert APA Table</Button></TabsContent><TabsContent value="figure" className="apa-pane"><label>Figure title<Input value={label} onChange={(e)=>setLabel(e.target.value)}/></label><Button onClick={()=>insertMarkup(`<figure class="apa-figure"><p><strong>Figura 1</strong><br><em>${label}</em></p><div class="figure-placeholder">Inserte aquí la imagen desde Content Collection</div><figcaption><em>Nota.</em> Adaptado de Autor (año). Incluya licencia o derechos cuando corresponda.</figcaption></figure>`)}>Insert APA Figure</Button></TabsContent></Tabs><div className="apa-checklist"><strong>APA 7 Checklist</strong><span>✓ Citations and references match</span><span>✓ DOI como enlace https://doi.org/…</span><span>✓ Numbered tables and figures</span><span>✓ Alternative text y notas accesibles</span></div></DialogContent></Dialog>;
}
